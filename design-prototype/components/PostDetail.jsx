// PostDetail — article view with TOC (active section highlighted via IntersectionObserver), share dropdown

function PostDetailPage({ postId, onNav }) {
  const { t, type } = useTheme();
  const post = window.POSTS.find(p => p.id === postId);
  const cat = post && window.findCat(post.cat);
  const sub = post && window.findSub(post.cat, post.sub);
    ...
    <div style={{ maxWidth: 1400, margin: '0 auto', padding: '40px 24px', display: 'grid', gridTemplateColumns: '220px 1fr 240px', gap: 40 }}>
      ...
      <aside style={{ fontFamily: type.body }}>
        <div style={{ position: 'sticky', top: 80 }}>
          <button onClick={() => onNav(post.cat === 'project' ? { route: 'sub', cat: post.cat, sub: post.sub } : { route: 'sub', cat: post.cat, sub: post.sub })}
          ...
          {post.series && (
            <div style={{ marginTop: 24, padding: 12, border: `1px solid ${t.border}`, borderRadius: 6, background: t.surfaceAlt }}>
              ...
              <div style={{ fontSize: 11, color: t.textSubtle, marginTop: 4 }}>Part 2 of 5</div>
            </div>
          )}
        </div>
      </aside>
      ...
      {/* Center: article */}
      <main style={{ minWidth: 0, maxWidth: 720, justifySelf: 'center' }}>
        <header style={{ marginBottom: 40 }}>
          <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
            <Tag>{cat?.name}</Tag><Tag>{sub?.name}</Tag>
            {post.tags.slice(0, 3).map(tag => <Tag key={tag}>#{tag}</Tag>)}
          </div>
          <h1 style={{ margin: 0, fontSize: 42, fontFamily: type.display, fontWeight: 700, letterSpacing: -1, lineHeight: 1.15, color: t.text, textWrap: 'balance' }}>{post.title}</h1>
          <p style={{ margin: '20px 0 0', fontSize: 18, color: t.textMuted, lineHeight: 1.6, textWrap: 'pretty' }}>{post.excerpt}</p>
        ...
        <article style={{ fontSize: 16.5, lineHeight: 1.75, color: t.text, fontFamily: type.body }}>
          {sections.map((s, i) => (
            <section key={s.id} id={s.id} style={{ marginBottom: 36, scrollMarginTop: 80 }}>
              <h2 style={{
                margin: '32px 0 16px', fontFamily: type.display, fontWeight: 700,
                color: t.text, letterSpacing: -0.4,
                fontSize: s.level === 1 ? 26 : 20,
              }}>{s.title}</h2>
              <DummyPara />
              {i % 2 === 0 && <DummyCode t={t} type={type} />}
              <DummyPara />
            </section>
          ))}
        </article>
        ...
        <div style={{ marginTop: 40, padding: 20, border: `1px solid ${t.border}`, borderRadius: 8, background: t.surfaceAlt, display: 'flex', gap: 16, alignItems: 'center' }}>
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: t.accent, color: t.accentText, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 22, fontFamily: type.display }}>J</div>
          <div style={{ flex: 1 }}>
              ...
              return (
                <a key={s.id} href={`#${s.id}`}
                   ...
                   style={{
                     ...
                     marginLeft: s.level === 2 ? 12 : 0,
                     borderLeft: active ? `2px solid ${t.accent}` : '2px solid transparent',
                     marginLeft: (s.level === 2 ? 12 : 0),
                     fontSize: s.level === 2 ? 12 : 12.5,
...
function DummyPara() {
  const { t } = useTheme();
  return <p style={{ margin: '0 0 16px', color: t.text, textWrap: 'pretty' }}>
    이 영역은 실제 글이 들어갈 자리에 대한 더미 텍스트입니다. 본문 타이포그래피와 여백, 그리고 코드 블록과 제목의 리듬을 확인하기 위한 플레이스홀더예요.
    문단을 한 번 더 이어서 본문이 두 줄 이상일 때의 줄 간격과 <code style={{ fontFamily: 'var(--font-mono)', background: t.codeBg, padding: '1px 5px', borderRadius: 3, fontSize: '0.9em' }}>inline code</code>
    의 색 대비를 확인합니다. 실제 글은 MDX로 작성돼 이 자리에 렌더링돼요.
  </p>;
}

function DummyCode({ t, type }) {
  const lines = [
  ...
  return (
    <pre style={{
      margin: '20px 0', padding: 16, background: t.codeBg,
      border: `1px solid ${t.border}`, borderRadius: 6,
      fontFamily: type.mono, fontSize: 13.5, color: t.code,
...
function ShareButton({ post }) {
  ...
  const items = [
    { label: '링크 복사', icon: <Icons.Copy size={13}/>, action: () => {
      try { navigator.clipboard.writeText(`https://readme.dev/${post.slug}`); } catch {}
      setCopied(true); setTimeout(() => { setCopied(false); setOpen(false); }, 1400);
    }},
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(x => !x)} style={{
        ...
        color: t.text, fontSize: 12, cursor: 'pointer', fontFamily: type.body,
      }}>
        <Icons.Share size={13} /> 공유
      </button>
      {open && (
        <div style={{
          position: 'absolute', top: '100%', right: 0, marginTop: 6, minWidth: 180,
          padding: 6, background: t.surface, border: `1px solid ${t.border}`,
          borderRadius: 6, boxShadow: window.TOKENS.shadow.lg, zIndex: 30,
        }}>
          {items.map((it, i) => (
            <button key={i} onClick={it.action} style={{
              ...
              fontSize: 12.5, fontFamily: type.body, borderRadius: 4, textAlign: 'left',
            }}
            onMouseEnter={e => e.currentTarget.style.background = t.surfaceAlt}
            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
              <span style={{ color: t.textMuted, display: 'flex' }}>
                {copied && i === 0 ? <Icons.Check size={13}/> : it.icon}
              </span>
              <span style={{ flex: 1 }}>{copied && i === 0 ? '복사됨' : it.label}</span>
            </button>
          ))}
        </div>
      )}
