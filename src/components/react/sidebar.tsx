...
export function Sidebar({ current, onNav, compact }: SidebarProps) {
  ...
  return (
    <aside style={{ width: compact ? 220 : 240, flexShrink: 0, padding: '8px 0', fontFamily: type.body }}>
      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, color: t.textSubtle, padding: '8px 14px 12px', textTransform: 'uppercase' }}>
        Categories
      </div>
      <nav>
        {CATEGORIES.map(cat => {
            ...
            <div key={cat.slug} style={{ marginBottom: 2 }}>
              <div style={{ display: 'flex', alignItems: 'stretch' }}>
                <button
                  ...
                  style={{ flex: 1, textAlign: 'left', padding: '7px 8px 7px 14px', background: isParentActive ? t.surfaceAlt : 'transparent', border: 'none', cursor: 'pointer', color: isParentActive ? t.text : t.textMuted, fontSize: 14.5, fontWeight: isParentActive ? 600 : 500, fontFamily: type.body, borderRadius: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ padding: '20px 14px 12px', borderTop: `1px solid ${t.border}`, marginTop: 8 }}>
        <button
          ...
          style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, color: t.textSubtle, textTransform: 'uppercase', marginBottom: 10, background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', gap: 6, fontFamily: type.body }}
      ...
      <div style={{ padding: '20px 14px 12px', borderTop: `1px solid ${t.border}` }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, color: t.textSubtle, textTransform: 'uppercase', marginBottom: 10 }}>
          Recently viewed
        </div>
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
      {tags.map(tag => (
        <button key={tag} onClick={() => onNav({ route: 'tags', tag })}
          style={{ padding: '2px 8px', fontSize: 12, fontFamily: type.mono, background: t.tag, color: t.tagText, border: `1px solid ${t.border}`, borderRadius: 3, cursor: 'pointer', fontWeight: 500 }}
      ...
      {recent.map(p => p && (
        <button key={p.id} onClick={() => onNav({ route: 'post', id: p.id })}
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontSize: 12.5, color: t.text, lineHeight: 1.35, fontWeight: 500, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as any }}>
              {p.title}
            </div>
          </div>
        </button>
